...
if provider.respond_to?(:span_processors)
  ...
  provider.span_processors.each_with_index do |processor, i|
    ...
    if processor.is_a?(DSPy::Observability::AsyncSpanProcessor)
      ...
      puts "     - Exporter class: #{processor.instance_variable_get(:@exporter).class}"
    end
  end
elsif provider.is_a?(OpenTelemetry::SDK::Trace::TracerProvider)
  puts "SDK Tracer Provider detected"
  # Try to access span processors through internal methods
  if provider.instance_variable_defined?(:@span_processors)
    processors = provider.instance_variable_get(:@span_processors)
    puts "Found #{processors.size} span processor(s)"
    processors.each_with_index do |processor, i|
      puts "  #{i}: #{processor.class}"
      if processor.is_a?(DSPy::Observability::AsyncSpanProcessor)
        puts "     - AsyncSpanProcessor detected"
        exporter = processor.instance_variable_get(:@exporter)
        puts "     - Exporter class: #{exporter.class}"
        puts "     - Exporter endpoint: #{exporter.instance_variable_get(:@uri)}" if exporter.respond_to?(:instance_variable_get)
      end
    end
  end
end
...
puts
puts "=== Waiting for async export (10 seconds) ==="
sleep 10
