...
RSpec.describe DSPy::LM::Adapters::Gemini::SchemaConverter do
  ...
  describe '.supports_structured_outputs?' do
    ...
    it 'returns false for JSON-only models (no schema support)' do
      # Based on official Google documentation: older models like gemini-pro are JSON-only
      expect(described_class.supports_structured_outputs?("gemini/gemini-pro")).to eq(false)
