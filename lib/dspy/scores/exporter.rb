...
module DSPy
  module Scores
    ...
    class Exporter
      ...
      sig { params(score_event: ScoreEvent).void }
      ...
      def subscribe_to_events
        @subscription_id = DSPy.events.subscribe('score.create') do |_name, attrs|
          ...
          score_event = ScoreEvent.new(
            id: attrs[:score_id],
            name: attrs[:score_name],
            value: attrs[:score_value],
            data_type: DataType.deserialize(attrs[:score_data_type]),
      ...
      sig { params(score_event: ScoreEvent).returns(T::Hash[Symbol, T.untyped]) }
